...
class MySolenso:
    ...
    Attributes:
        ...
        oempowercount (MySolensoOEMPowerCount): OEM aggregated PV and consumption
            ...
            to set the date range.
        stationinfodevice (MySolensoStationInfoDevice): Full device tree for the
            active station. Returns the DTU and all attached microinverters with
            serial numbers, firmware versions, and connectivity status.
        stationcountdevice (MySolensoStationCountDevice): Device count summary for
            the active station. Exposes counts by device type (DTU, microinverters,
            repeaters, meters, BMS, etc.).
        stationak (MySolensoStationAK): Geographic and address information for the
            active station. Exposes :attr:`longitude`, :attr:`latitude`, and
            :attr:`address`.
        dtuselectall (MySolensoDTUSelectAll): DTU and associated microinverter list
            for the active station. Use :attr:`list_micros` for a minimal
            ``{sn, id}`` projection or :attr:`list_micros_info` for the full records.
        dtufind (MySolensoDTUFind): Detailed record for a single DTU. Call
            :meth:`~mysolenso.services.dtu.find.MySolensoDTUFind.set_dtu` with
            the DTU id before accessing :attr:`all_data`.
